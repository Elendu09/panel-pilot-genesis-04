...
import { 
  CreditCard, Check, Zap, Crown, Wallet, ArrowUpRight, Calendar,
  DollarSign, Sparkles, Percent, Loader2, AlertTriangle, ArrowDownRight
} from "lucide-react";
...
const plans = [
  {
    ...
    features: [
      'Up to 50 Services', 'Basic Analytics', 'Email Support',
      'Subdomain Only', '100 Orders/month'
    ],
  },
  {
    name: 'Basic',
    ...
    popular: true,
    features: [
      'Up to 5,000 Services', 'Full Analytics Dashboard', 'Priority Email Support',
      'Custom Domain', '1,000 Orders/month', 'API Access', 'Custom Branding'
    ],
  },
  {
    name: 'Pro',
    ...
    features: [
      'Up to 10,000 Services', 'Advanced Analytics + Reports', '24/7 Priority Support',
      'Multiple Custom Domains', 'Unlimited Orders', 'Full API Access',
      'White-label Branding', 'Dedicated Account Manager', 'Custom Integrations'
    ],
  }
];
...
const Billing = () => {
  ...
  const [commissionData, setCommissionData] = useState<CommissionData>({
    commissionRate: 5, earnedThisMonth: 0, pendingCommission: 0, paidCommission: 0,
  });
    ...
    if (panel?.id) {
      ...
      if (isSuccess || isCancelled || transactionId) {
        ...
        if (isCancelled) {
          ...
          if (transactionId) fetchBillingData();
          ...
          const lookupAndVerify = async () => {
            try {
              ...
              if (recentTx?.id) {
                ...
                if (data?.status === 'completed') {
                  toast({ title: data?.subscriptionUpdated ? 'Subscription Activated!' : 'Payment Successful!', description: data?.subscriptionUpdated ? 'Your plan has been upgraded successfully.' : `$${Number(data.amount).toFixed(2)} has been added to your balance.` });
                  fetchBillingData();
                } else if (data?.status === 'failed') {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', {
          ...
          if (newStatus === 'completed') {
            ...
            toast({ 
              title: meta?.type === 'subscription' ? 'Subscription Activated!' : 'Payment Completed!',
              description: meta?.type === 'subscription' 
                ? `Your ${meta.plan || 'new'} plan has been activated successfully.`
                : `$${(payload.new as any).amount?.toFixed(2) || '0.00'} has been added to your balance.`
            });
            fetchBillingData();
          } else if (newStatus === 'failed') {
            toast({ variant: 'destructive', title: 'Payment Failed', description: 'Your payment could not be processed. Please try again or use a different payment method.' });
            fetchBillingData();
          }
        })
        .subscribe();
  ...
  const fetchBillingData = async () => {
    if (!panel?.id) return;
    try {
      const { data: freshPanel } = await supabase.from('panels').select('balance').eq('id', panel.id).single();
      setPanelBalance(freshPanel?.balance || 0);

      const { data: sub } = await supabase.from('panel_subscriptions').select('*').eq('panel_id', panel.id).maybeSingle();
      ...
      const { data: fees } = await supabase.from('platform_fees').select('fee_amount, created_at').eq('panel_id', panel.id);
      if (fees) {
  ...
  const handleUpgrade = async (planName: string) => {
    ...
    // Has enough balance - offer balance payment
    ...
    // Not enough balance - gateway payment
  ...
  // FIX: Added billingCycle and plan to balance payment request
  const handleBalanceUpgrade = async () => {
    if (!pendingUpgradePlan || !panel?.id || !profile?.id) return;

    const price = getPlanPrice(pendingUpgradePlan);
    const planType = pendingUpgradePlan.name.toLowerCase();
  ...
  // FIX: Added all required parameters including billingCycle, plan, amount
  const proceedWithGatewayPayment = async (plan: typeof plans[0], selectedGateway?: string) => {
    if (!panel?.id || !profile?.id) {
      toast({ variant: 'destructive', title: 'Session Error', description: 'Panel or profile not loaded. Please refresh.' });
      return;
    }
    ...
    if (price <= 0) {
      toast({ variant: 'destructive', title: 'Invalid Amount', description: 'Plan price must be greater than zero.' });
      return;
    }
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Upgrade Failed', description: error.message || 'Failed to initiate payment. Please try again.' });
    } finally {
  ...
  const handleDeposit = async (amount: number, method: string) => {
    if (!panel?.id || !profile?.id) {
      toast({ variant: 'destructive', title: 'Session Error', description: 'Panel or user session not available. Please refresh the page.' });
      return;
    }
    if (!method) {
      toast({ variant: 'destructive', title: 'No Payment Method', description: 'Please select a payment method to continue.' });
      return;
    }
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Deposit Failed', description: error.message || 'Failed to initiate deposit. Please check your payment gateway configuration.' });
    } finally {
  ...
  const handlePayCommission = async () => {
    ...
    if (!gatewayToUse) {
      toast({ variant: 'destructive', title: 'Payment Methods Unavailable', description: 'Platform payment methods are not configured yet. Please contact the administrator.' });
      return;
    }
    try {
      const { data, error } = await supabase.functions.invoke('process-payment', {
        body: {
          ...
          returnUrl: `${window.location.origin}/panel/billing`,
          cancelUrl: `${window.location.origin}/panel/billing?cancelled=true`,
          metadata: { type: 'commission_payment', panelId: panel.id }
        }
      });
      if (error) throw error;
      if (data?.redirectUrl) window.location.href = data.redirectUrl;
    } catch (error) {
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to initiate commission payment' });
    }
  };
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      ...
      {isExpired && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border-2 border-destructive/50 bg-destructive/10 p-6 text-center space-y-3">
          <AlertTriangle className="w-10 h-10 text-destructive mx-auto" />
          <h2 className="text-xl font-bold text-destructive">Subscription Expired</h2>
          <p className="text-muted-foreground max-w-md mx-auto">
            Your <span className="font-semibold capitalize">{currentPlan}</span> plan has expired. Please renew your subscription to regain full access.
          </p>
        </motion.div>
      )}
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-card/60 backdrop-blur-xl border-border/50 relative overflow-hidden">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-2"><Wallet className="w-4 h-4" /><span className="text-sm">Panel Balance</span></div>
        ...
        <Card className="bg-card/60 backdrop-blur-xl border-border/50 relative overflow-hidden">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-2"><Percent className="w-4 h-4" /><span className="text-sm">Commission Due</span></div>
            <p className="text-2xl lg:text-3xl font-bold text-orange-500">${commissionData.pendingCommission.toFixed(2)}</p>
          </CardContent>
        </Card>
        <Card className="bg-card/60 backdrop-blur-xl border-border/50 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-primary/10 rounded-full blur-2xl" />
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-2"><CreditCard className="w-4 h-4" /><span className="text-sm">Current Plan</span></div>
            <div className="flex items-center gap-2">
              <p className="text-2xl lg:text-3xl font-bold capitalize">{currentPlan}</p>
              <Badge className={cn("capitalize", subscription?.status === 'active' && "bg-emerald-500/20 text-emerald-500", subscription?.status === 'expired' && "bg-destructive/20 text-destructive")}>
        ...
        <Card className="bg-card/60 backdrop-blur-xl border-border/50 relative overflow-hidden">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-muted-foreground mb-2"><Calendar className="w-4 h-4" /><span className="text-sm">Next Billing</span></div>
            <p className="text-2xl lg:text-3xl font-bold">${getPlanPrice(plans.find(p => p.name.toLowerCase() === currentPlan) || plans[0])}</p>
            <p className="text-xs text-muted-foreground">{subscription?.expires_at ? new Date(subscription.expires_at).toLocaleDateString() : 'N/A'}</p>
          </CardContent>
        </Card>
      </motion.div>
            ...
            return (
              <motion.div key={plan.name} variants={itemVariants} className="relative">
                ...
                <Card className={cn(
                  ...
                  <div className={cn("absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl opacity-30", `bg-gradient-to-br ${plan.color}`)} />
                  <CardHeader className="text-center pb-2">
                    <div className={cn("w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-3 bg-gradient-to-br", plan.color)}>
                  ...
                  <CardContent className="space-y-6">
                    <div className="text-center">
                      ...
                      {billingCycle === 'yearly' && getYearlySavings(plan) > 0 && (
                        <div className="mt-1">
                          <Badge variant="outline" className="text-xs text-emerald-500 border-emerald-500/30">Save ${getYearlySavings(plan)}/year</Badge>
                        </div>
                      )}
                    </div>
                    ...
                    <ul className="space-y-3">
                      {plan.features.map((feature, i) => (
                        <li key={i} className="flex items-center gap-2 text-sm">
                          <Check className="w-4 h-4 text-emerald-500 shrink-0" /><span>{feature}</span>
                        </li>
                      ))}
                    </ul>
                    <Button
                      className={cn(
                        ...
                        (isCurrent && !isRenewable) && "bg-muted text-muted-foreground hover:bg-muted"
                      )}
                    ...
                    >
                      {isLoading ? (<><Loader2 className="w-4 h-4 animate-spin" />Processing...</>) 
                        : isRenewable ? (<><Crown className="w-4 h-4" />Renew Plan</>) 
                        : isCurrent ? ('Current Plan') 
                        : isLowerTier && isActivePaid ? (<><ArrowDownRight className="w-4 h-4" />Downgrade</>) 
      ...
      <AlertDialog open={downgradeDialogOpen} onOpenChange={setDowngradeDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2"><ArrowDownRight className="w-5 h-5 text-amber-500" />Downgrade Plan?</AlertDialogTitle>
            <AlertDialogDescription className="space-y-2">
              <p>Your current <span className="font-semibold capitalize">{currentPlan}</span> plan will remain active until{' '}
                <span className="font-semibold">{subscription?.expires_at ? new Date(subscription.expires_at).toLocaleDateString() : 'the end of your billing period'}</span>.
              </p>
              <p>After that, you'll be switched to the <span className="font-semibold capitalize">{pendingDowngradePlan}</span> plan.</p>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={async () => {
                ...
                try {
                  await supabase.from('panel_subscriptions').update({ pending_downgrade: pendingDowngradePlan }).eq('panel_id', panel.id);
            ...
            >Confirm Downgrade</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={balanceDialogOpen} onOpenChange={setBalanceDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2"><Wallet className="w-5 h-5 text-emerald-500" />Pay from Balance?</AlertDialogTitle>
            <AlertDialogDescription className="space-y-2">
              <p>You have <span className="font-semibold text-emerald-500">${panelBalance.toFixed(2)}</span> in your panel balance. 
          ...
          <AlertDialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button variant="outline" onClick={() => {
              ...
              <CreditCard className="w-4 h-4 mr-2" />Pay via Gateway
            </Button>
            <Button onClick={handleBalanceUpgrade} disabled={balancePaymentLoading} className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700">
              {balancePaymentLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Wallet className="w-4 h-4 mr-2" />}
