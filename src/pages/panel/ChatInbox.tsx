...
const ChatInbox = ({ embedded = false }: ChatInboxProps) => {
  ...
  // Ref to track previous sessions JSON to prevent unnecessary re-renders
  ...
  useEffect(() => {
    const fetchPanelId = async () => {
      ...
      let panelQuery = supabase.from("panels").select("id").eq("owner_id", profile.id);
  ...
  // Fetch chat sessions - with smart update to prevent focus loss
  const fetchSessions = useCallback(async () => {
    ...
    // Only update state if data actually changed to prevent re-renders that steal focus
    const newJson = JSON.stringify(sessionsWithDetails);
    if (prevSessionsRef.current !== newJson) {
      prevSessionsRef.current = newJson;
  ...
  const sendMessage = useCallback(
    async (content?: string) => {
      ...
      try {
        ...
        // Restore focus after send without triggering re-render
  ...
  // Memoized Session Card to prevent re-renders
    ...
    >
      <div className="flex items-start gap-3">
        <Avatar className="w-10 h-10 shrink-0">
          <AvatarFallback className="bg-primary/10 text-primary">{session.visitor_name?.[0] || "V"}</AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-1">
            <span className="font-medium truncate">{session.visitor_name || "Visitor"}</span>
            <span className="text-xs text-muted-foreground shrink-0 ml-2">{formatTime(session.last_message_at)}</span>
          </div>
          <p className="text-sm text-muted-foreground truncate">{session.last_message}</p>
        </div>
        {(session.unread_count || 0) > 0 && <Badge className="shrink-0">{session.unread_count}</Badge>}
      </div>
    </div>
  ));
  SessionCard.displayName = "SessionCard";

  // Memoized Chat Messages to prevent re-renders stealing focus
          ...
          <div key={msg.id} className={cn("flex gap-2", msg.sender_type === "owner" ? "flex-row-reverse" : "")}>
            ...
            >
              ...
              <div className={cn("flex items-center gap-1 mt-1", msg.sender_type === "owner" ? "justify-end" : "")}>
                ...
                {msg.sender_type === "owner" && (
                  <CheckCheck className={cn("w-3 h-3", msg.is_read ? "text-white/80" : "text-white/40")} />
                )}
              </div>
            </div>
          </div>
        ...
        {visitorTyping && (
          <div className="flex items-center gap-2">
            <Avatar className="w-8 h-8">
              <AvatarFallback className="bg-muted">{selectedSession?.visitor_name?.[0] || "V"}</AvatarFallback>
            </Avatar>
      ...
      <div className="space-y-4 pb-20">
          ...
          <SheetContent side="bottom" className="h-[90vh] rounded-t-2xl p-0 flex flex-col">
            <SheetHeader className="p-4 border-b shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <SheetTitle className="text-left text-base">{selectedSession?.visitor_name || "Visitor"}</SheetTitle>
        ...
        <Dialog open={cannedDialogOpen} onOpenChange={setCannedDialogOpen}>
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingCanned ? "Edit Quick Reply" : "Quick Replies"}</DialogTitle>
              <DialogDescription>Create and manage quick reply templates.</DialogDescription>
            </DialogHeader>
                    ...
                    {cannedResponses.map((response) => (
                      <div key={response.id} className="flex items-center justify-between p-3 border rounded-lg">
                        ...
                        <div className="flex items-center gap-1 ml-2">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => deleteCannedResponse(response.id)}>
    ...
    <div className={cn("flex gap-4", embedded ? "h-[calc(100vh-200px)]" : "h-[calc(100vh-120px)]")}>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-medium truncate">{session.visitor_name || "Visitor"}</span>
                        <span className="text-xs text-muted-foreground">{formatTime(session.last_message_at)}</span>
                      </div>
                      <p className="text-sm text-muted-foreground truncate">{session.last_message}</p>
                    </div>
                    {(session.unread_count || 0) > 0 && <Badge className="shrink-0">{session.unread_count}</Badge>}
        ...
        {selectedSession ? (
          ...
          <CardContent className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Choose a chat from the list to start responding</p>
            </div>
          </CardContent>
        )}
      </Card>
      ...
      <Dialog open={cannedDialogOpen} onOpenChange={setCannedDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{editingCanned ? "Edit Quick Reply" : "Quick Replies"}</DialogTitle>
            <DialogDescription>Create and manage quick reply templates for faster responses.</DialogDescription>
          </DialogHeader>
          ...
          {editingCanned || cannedResponses.length === 0 ? (
            <div className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button onClick={saveCannedResponse}>{editingCanned ? "Update" : "Create"} Quick Reply</Button>
              </DialogFooter>
            </div>
          ) : (
            <div className="space-y-4">
                  ...
                  {cannedResponses.map((response) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground truncate">{response.content}</p>
                      </div>
                      <div className="flex items-center gap-1 ml-2">
                        <Button
                          variant="ghost"
