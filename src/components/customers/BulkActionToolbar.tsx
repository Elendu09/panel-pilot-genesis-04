    ...
    <AnimatePresence>
      {selectedCount > 0 && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 20 }}
          className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100vw-2rem)] max-w-fit"
        >
          <div className="flex items-center gap-2 sm:gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-background/95 backdrop-blur-xl border border-border/50 rounded-2xl shadow-2xl overflow-x-auto scrollbar-hide">
            ...
            <Button variant="ghost" size="icon" onClick={onActivate} className="h-9 w-9 text-green-500 hover:text-green-600 shrink-0">
              <UserCheck className="w-4 h-4" />
            </Button>
            
            <Button variant="ghost" size="icon" onClick={onSuspend} className="h-9 w-9 text-destructive hover:text-destructive shrink-0">
