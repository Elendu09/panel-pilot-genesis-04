...
export function PanelSwitcher({ collapsed = false }: PanelSwitcherProps) {
  const { panel, allPanels, switchPanel, canCreatePanel, getMaxPanels } = usePanel();
  const navigate = useNavigate();
  const [switching, setSwitching] = useState(false);

  // Don't show switcher if only one panel and can't create more
  if (allPanels.length <= 1 && !canCreatePanel()) return null;
        ...
        <DropdownMenuContent side="right" align="start" className="w-56">
          ...
          {allPanels.map((p) => (
            ...
            >
              ...
              {p.id === panel?.id && <Check className="w-3.5 h-3.5 text-primary" />}
            </DropdownMenuItem>
          ))}
          {canCreatePanel() && (
            <>
      ...
      <DropdownMenuContent align="start" className="w-[var(--radix-dropdown-menu-trigger-width)] min-w-[200px]">
        ...
        {allPanels.map((p) => (
          ...
          >
            ...
            {p.id === panel?.id && <Check className="w-3.5 h-3.5 text-primary shrink-0" />}
          </DropdownMenuItem>
        ))}
        {canCreatePanel() && (
          <>
