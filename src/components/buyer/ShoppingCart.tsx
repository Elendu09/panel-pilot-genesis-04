...
const ShoppingCart = ({
  ...
  const handleBulkCheckout = async () => {
    ...
    if (errors.length > 0) {
      toast({ variant: "destructive", title: "Validation Error", description: errors[0] });
      return;
    }
    ...
    try {
      ...
      toast({ variant: "destructive", title: "Checkout Failed", description: "Some orders failed. Please try again." });
    } finally {
  ...
  const handleBulkAdd = async (
    items: { service: any; quantity: number; targetUrl: string; effectivePrice: number }[],
  ) => {
    if (!buyerId || !panelId) {
    ...
    try {
      for (const item of items) {
        await supabase.from("buyer_cart").insert({
          buyer_id: buyerId,
          panel_id: panelId,
          service_id: item.service.id,
          quantity: item.quantity,
      ...
      toast({ title: "Added to cart", description: `${items.length} item${items.length > 1 ? "s" : ""} added` });
      setActiveTab("cart");
    } catch (error) {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col p-0 overflow-hidden">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
          <TabsList className="mx-4 sm:mx-6 mt-4 grid grid-cols-3 text-[10px] sm:text-xs shrink-0">
            <TabsTrigger value="cart" className="gap-1 sm:gap-1.5 px-1 sm:px-2">
              <CartIcon className="w-3 h-3 sm:w-3.5 sm:h-3.5" />({cart.length})
            </TabsTrigger>
          ...
          {/* Cart Tab */}
          <TabsContent value="cart" className="flex-1 flex flex-col mt-0 min-h-0 data-[state=active]:flex">
            {cart.length === 0 ? (
              <div className="flex-1 flex items-center justify-center px-6">
                <div className="text-center">
                  <CartIcon className="w-12 h-12 mx-auto mb-4 text-muted-foreground" />
                  <p className="text-muted-foreground">{t("cart.empty")}</p>
                  <p className="text-sm text-muted-foreground mt-1">{t("services.add_to_cart")}</p>
                  <Button variant="outline" size="sm" className="mt-4" onClick={() => setActiveTab("bulk")}>
                ...
                <div className="border-t p-6 space-y-4 shrink-0">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={cn("font-medium", hasEnoughBalance ? "text-green-500" : "text-destructive")}>
                        {formatPrice(buyerBalance)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-medium">
                        {t("common.total")} ({cart.length} {t("cart.items")})
                      </span>
          ...
          {/* Bulk Add Tab - FIXED: Content starts at top */}
          <TabsContent value="bulk" className="flex-1 mt-0 min-h-0 overflow-auto data-[state=active]:block">
          ...
          {/* Quick Repeat Tab - FIXED: Content starts at top */}
          <TabsContent value="repeat" className="flex-1 mt-0 min-h-0 overflow-auto data-[state=active]:block">
            <div className="p-4 sm:p-6">
              <QuickRepeatOrder
                services={services}
                getEffectivePrice={getEffectivePrice}
