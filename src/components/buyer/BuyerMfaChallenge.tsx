...
interface BuyerMfaChallengeProps {
  open: boolean;
  buyerId: string;
  panelId: string;
  token: string;
...
export function BuyerMfaChallenge({ open, buyerId, panelId, token, onVerified, onCancel }: BuyerMfaChallengeProps) {
  ...
  const handleVerify = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('buyer-auth', {
        body: { action: 'mfa-validate', panelId, buyerId, token, mfaToken: code }
      });
  ...
  const handleBackupCode = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('buyer-auth', {
        body: { action: 'mfa-backup', panelId, buyerId, token, backupCode: backupCode.trim() }
      });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      if (data?.valid) {
      ...
      <DialogContent className="sm:max-w-md" onPointerDownOutside={(e) => e.preventDefault()}>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <Button variant="link" size="sm" className="text-xs p-0 h-auto text-destructive" onClick={onCancel}>
              Sign out
            </Button>
          </div>
        </div>
