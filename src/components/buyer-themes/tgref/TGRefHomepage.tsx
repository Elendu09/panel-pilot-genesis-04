    ...
    <div className="min-h-screen font-mono" style={{ backgroundColor: bgColor, color: textCol }}>
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: containerMax }}>
          <div className="flex items-center justify-between h-16">
            <Link to="/" className="flex items-center gap-2">
              {displayLogo ? (
                <img src={displayLogo} alt={companyName} className="w-8 h-8 rounded object-contain" />
              ) : (
      ...
      {showFAQs && faqs.length > 0 && (
        <section style={{ paddingTop: sectionPadding, paddingBottom: sectionPadding, backgroundColor: surfaceColor }}>
          <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: containerMax }}>
            <motion.div {...(enableAnimations ? { initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 } } : {})} className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                <span style={{ color: primary }}>&gt;</span> man --help
              </h2>
              <p style={{ color: mutedColor }}>Frequently asked questions</p>
            </motion.div>
                ...
                {faqs.map((faq, idx) => (
                  <AccordionItem key={idx} value={`faq-${idx}`} className="rounded-lg px-6 font-mono" style={{ backgroundColor: bgColor, border: `1px solid ${primary}1a` }}>
                    <AccordionTrigger className="text-left font-semibold hover:no-underline">
                      <span style={{ color: primary }}>$</span> {faq.question}
                    </AccordionTrigger>
      ...
      <section style={{ paddingTop: sectionPadding, paddingBottom: sectionPadding }}>
        <div className="mx-auto px-4" style={{ maxWidth: 900 }}>
          <motion.div {...(enableAnimations ? { initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 } } : {})} className="text-center">
            ...
            <Button size="lg" asChild className="font-bold font-mono text-lg px-10" style={primaryButtonStyle}>
              <Link to="/auth?tab=signup">&gt; ./signup --free</Link>
            </Button>
          </motion.div>
        </div>
      </section>
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8" style={{ maxWidth: containerMax }}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono" style={{ color: mutedColor }}>
