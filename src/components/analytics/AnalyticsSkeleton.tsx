    ...
    <CardContent>
      <div className="flex md:grid md:grid-cols-5 gap-3 overflow-x-auto pb-2">
        {[1, 2, 3, 4, 5].map((i) => (
          <div 
            key={i} 
            className="flex-shrink-0 min-w-[112px] md:min-w-0 p-3 rounded-xl bg-muted/30 space-y-2"
          >
            <div className="flex items-center gap-2">
              <Skeleton className="h-2 w-2 rounded-full bg-primary/20" />
              <Skeleton className="h-3 w-16 bg-primary/10" />
            </div>
            <Skeleton className="h-6 w-12 bg-primary/15" />
  ...
  <Card className="bg-card/80 backdrop-blur-sm border-border/50 relative overflow-hidden">
    ...
    <CardContent className="space-y-4">
      ...
      <div className="space-y-2 pt-2 border-t border-border/50">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center justify-between py-1.5">
            <div className="flex items-center gap-2">
              <Skeleton className="h-6 w-6 rounded bg-primary/10" />
              <Skeleton className="h-4 w-24 bg-primary/10" />
            </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border/50">
        ...
        <Skeleton className="h-6 w-24 bg-emerald-500/20" />
      </div>
    </CardContent>
  </Card>
);
...
const ChartSkeleton = () => (
  <Card className="bg-card/80 backdrop-blur-sm border-border/50 relative overflow-hidden">
    ...
    <CardContent>
      <div className="h-[220px] md:h-[280px] w-full flex items-end gap-2 p-4">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex items-center gap-1 bg-muted/50 rounded-lg p-1">
            {[1, 2, 3, 4, 5].map((i) => (
              <Skeleton key={i} className="h-8 w-16 rounded bg-primary/10" />
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {[1, 2, 3].map((i) => (
            <Skeleton key={i} className="h-10 w-28 rounded-lg bg-primary/10" />
          ))}
        </div>
      </div>
